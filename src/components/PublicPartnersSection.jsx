...
const PUBLIC_PARTNERS = [
  {
    id:'suiza-en-espanol',
    render:placement => <SuizaEspanolPartnerPromo key="suiza-en-espanol" placement={placement} variant="public-featured" />,
  },
  {
    id:'virtus360',
...
export default function PublicPartnersSection({ placement = 'public_landing' }) {
  ...
  const partnerCards = useMemo(() => rotateItems([
    ...
    ...PUBLIC_PARTNERS.filter(partner => partner.id !== 'punto-hispano' || !businessPartners.some(business => business.id === PUNTO_HISPANO_PROVIDER_ID)).map(partner => ({
