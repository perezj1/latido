...
function adminMessage(order:ClubOrder) {
  ...
  const text = [
    ...
    `Envío: ${money(order.shipping_amount, order.currency)}`, '',
...
function customerMessage(order:ClubOrder) {
  ...
  const text = [`Hola ${order.customer_name || ''},`, '', `Hemos recibido el pago de tu pedido ${order.reference}.`, '', summary, '', `Total: ${money(order.total_amount, order.currency)}`, '', 'Prepararemos el producto bajo demanda con Gelato. Recibirás otro mensaje cuando esté enviado.', '', 'Gracias,', 'Latido Club'].join('\n')
...
async function paidOrder(session:Stripe.Checkout.Session) {
  ...
  if (order.customer_email && !order.customer_email_sent_at) {
    ...
    await sendMail({ to:order.customer_email, ...message })
